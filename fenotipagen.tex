...
\begin{document}
\title{Automated Identification of the R7 Stage in Soybeans Using AI Techniques}
\newif\iffinal
\finaltrue
\iffinal
    % \author{
...
\label{fig:trainVal}
\end{figure}
For the training of CNN and Random Forest (RF) models, 2,301 images of different phenological stages were selected for the training set and 2,280 images for the validation set, as illustrated in ~\ref{fig:trainVal}. These images played an important role in ensuring that the models were capable of generalizing and performing accurate classifications, especially regarding phenological stages above R7. The images were carefully captured, taking into consideration the need to avoid the presence of unwanted elements, such as shadows, trees, buildings, and sky, which could introduce noise and compromise the quality of the training. 

\begin{figure}[!h]
    \centering
    \includegraphics[width=0.5\textwidth]{figs/createvgg16.png}\\
    \includegraphics[width=0.5\textwidth]{figs/createinceptionv3.png}\\
    \includegraphics[width=0.5\textwidth]{figs/createresnet.png}
    \caption{Implemented Neural Networks VGG16, Inception3, ResNet50.}
    \label{fig:VGG16_ResNet50}
\end{figure}

~\ref{fig:VGG16_ResNet50} shows the implementation of the VGG16, Inception3, and ResNet50 CNNs. These CNN architectures are popular and contain particularities that make them different and are used for different types of image classification problems. VGGNet, developed by Simonyan and Zisserman in 2015, is a CNN architecture that exclusively uses 3×3 filters in its convolutional layers. The VGG16 version, composed of 16 layers, demonstrated high accuracy in image classification. However, its depth results in high memory consumption and significant computational demand \cite{goodfellow2016deep}. 
...
\begin{figure}[!h]
\centering
\includegraphics[width=2.5in]{figs/exemploResnet50codigo.png}
\caption{Random Forest feature extraction model}
\label{fig:randomforest}
\end{figure}

Random Forest, as illustrated in code~\ref{code:extract_features}, is a machine learning model based on decision trees. It builds multiple trees and combines their results to obtain more robust predictions \cite{hastie2009elements}.
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.2\textwidth]{figs/ResnetABsolut.png}
%     \hspace{1cm} % espaço horizontal opcional entre as imagens
%     \includegraphics[width=0.2\textwidth]{figs/Resnet.png}
%     \caption{Confusion Matrix – ResNet50.}
%     \label{fig:ResNet50}
% \end{figure}
\begin{figure}[!h]
\centering
\includegraphics[width=2.5in]{figs/intercecion_v3.png}
\caption{Confusion Matrix – ResNet50.}
\label{fig:ResNet50}
\end{figure}
The ResNet50 model achieved a training accuracy of 94.51\%. However, its validation accuracy dropped to 73.55\%, suggesting potential overfitting on the training data. The confusion matrix for ResNet50, shown in ~\ref{fig:ResNet50}, revealed a balanced distribution between the classes, with a correct classification rate of 42\% for the 'Other' class and 58\% for the 'R7' class. These results indicate challenges in accurate class identification, leading to suboptimal model performance. 
\begin{figure}[!h]
\centering
\includegraphics[width=2.5in]{figs/intercecion_v3.png}
\caption{Confusion Matrix –  Inception V3.  }
\label{fig:InceptionV3}
\end{figure}
The InceptionV3 model demonstrated excellent performance on the training dataset, achieving an accuracy of 98.37\%. However, the validation accuracy decreased to 70.75\%, indicating overfitting. As illustrated in ~\ref{fig:InceptionV3}, the confusion matrix shows a similar pattern to that of ResNet50, with a 41\% accuracy for the 'Other' class and 59\% for the 'R7' class. The false positive rate was 59\%, while the false negative rate was 41\%. InceptionV3 also struggles with distinguishing between classes, particularly in classifying instances of the 'Other' class. 
\begin{figure}[!h]
\centering
\includegraphics[width=2.5in]{figs/intercecion_vGG16.png}
